...
if __name__ == "__main__":
    ...
    loader = DataLoader(MaestroDataset(configs), batch_size=configs.gpus*configs.batch_size, shuffle=True, num_workers=4)
    model = Transformer().to(configs.device)
    if configs.gpus>1:
        model = nn.DataParallel(model)
    criterion = nn.CrossEntropyLoss(ignore_index=PAD_IDX)         # 忽略 占位符 索引为500.
    step = 0
    write_graph = True
    ...
    for epoch in range(40):
        for enc_inputs, enc_inputs_mask, dec_inputs, dec_outputs in loader:  # enc_inputs : [batch_size, src_len]
            ...
            enc_inputs_mask = enc_inputs_mask.to(configs.device)
            ...
            if write_graph:
                if configs.gpus>1:
                    writer.add_graph(model.module, (enc_inputs[:configs.batch_size, ], 
                                enc_inputs_mask[:configs.batch_size, ], dec_inputs[configs.batch_size:, ]))
                else:
                    writer.add_graph(model, (enc_inputs, enc_inputs_mask, dec_inputs))
                write_graph = False

            outputs, enc_self_attns, dec_self_attns, dec_enc_attns = model(enc_inputs, enc_inputs_mask, dec_inputs)
